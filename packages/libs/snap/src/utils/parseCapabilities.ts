...
const hasField = <T extends string>(
...
const knownCapabilities: KnownCapMap = {
  'coin.TRANSFER_XCHAIN': ({ args }, { meta: { chainId } }, accounts) => {
    if (!Array.isArray(args))
      throw new InvalidRequestError(`${args} capability args was not an array`);
  ...
  'coin.GAS': (_, { meta: { gasLimit, gasPrice } }) =>
    `Gas spend:\nUp to ${gasLimit * gasPrice} KDA`,
  _unknown: ({ name, args }, { meta: { chainId } }) => {
    const strs = [
      `⚠️  **Unidentified capability.** Review carefully:\nCapability: ${name}`,
    ];
    if (args.length) {
      strs.push(
        `Arguments: ${JSON.stringify(
          args.map((a) => parseArg(a)),
          null,
          2,
        )}`,
      );
...
export const parseCapabilities = (
  ...
  return (
    signer.clist?.map((clist) => parseCapability(clist, txn, accounts)) ?? []
  );
};
