  ...
  return (
    <span>
      {chain.metaData.status === 'online' ? (
        <span
          style={{ color: tokens.kda.foundation.color.semantic.positive.n50 }}
        >
          (Online)
        </span>
      ) : chain.metaData.status === 'syncing' ? (
        <span style={{ color: tokens.kda.foundation.color.semantic.info.n50 }}>
          (Syncing)
        </span>
      ) : (
        <span
          style={{ color: tokens.kda.foundation.color.semantic.negative.n50 }}
        >
          (Offline)
        </span>
      )}
...
export const DataIndicator = ({
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: convertColorToBackgroundSubtleColor(
          color,
          undefined,
    ...
    >
      ...
      <LinesGraph
        ...
        options={{
          ...options,
          color: {
            line: tokens.kda.foundation.color.categorical.category1.default,
          },
        }}
      />
    </div>
...
export const BlocksGraph = ({
  ...
  if (!data)
    return (
      <div
        className={[styles.boxGraphWrapper, className]
          .filter(Boolean)
          .join(' ')}
      >
        No data
  ...
  return (
    ...
    >
      <LinesGraph
        chartId="blocks-graph"
        title={title}
        data={data}
        options={{
          ...options,
          color: {
            line: tokens.kda.foundation.color.categorical.category2.default,
          },
        }}
      />
    </div>
...
export const AddressesGraph = ({
  ...
  return (
    ...
    >
      <LinesGraph
        chartId="addresses-graph"
        title={title}
        data={data}
        options={{
          ...options,
          color: {
            line: tokens.kda.foundation.color.categorical.category3.default,
          },
        }}
      />
    </div>
...
export const ChainsGraph = ({
  ...
  const drawGraph = () => {
    ...
    progressPath
      ...
      .attrTween('d', function () {
        const interpolate = d3.interpolate(
          0,
          (activeChainCount / chainCount) * Math.PI,
        );
        const arc = (t: number) =>
          d3
            .arc()
    ...
    const getColors = () => {
      const colorStart = getComputedColor(
...
export const LinesGraph = ({
  ...
  const [interactingChartId, setInteractingChartId] = useState<string | null>(
    null,
  );

  let focus: d3.Selection<SVGGElement, unknown, null, undefined>;
  let tooltip: d3.Selection<HTMLDivElement, unknown, HTMLElement, any> | null =
    null;
  ...
  function drawGraph(isAnimated = false) {
    if (!svgRef.current || !data || data.length === 0 || !width || !height)
      return;
    ...
    const YScale = d3
      .scaleLinear()
      .domain(
        d3.extent(data, (d) => (typeof d.value === 'number' ? d.value : 0)) as [
          number,
          number,
        ],
      )
    ...
    const path = g
      ...
      .attr(
        'stroke',
        options.color?.line ??
          tokens.kda.foundation.color.categorical.category1.default,
      )
      .attr('stroke-width', 2)
      .attr('d', line);
    ...
    overlay
      .on('mouseenter', () => {
        const { focus: _focus, tooltip: _tooltip } = appendGraphTooltip(
          chartId,
          g,
          height,
          options,
      ...
      .on('mousemove', function (event) {
        setInteractingChartId((prevId) =>
          prevId !== chartId ? chartId : prevId,
        );

        const point = moveGraphTooltip(
          chartId,
          event,
          chainId,
          data,
    ...
    if (
      ...
      if (
        ...
        const pointIndex = data.findIndex(
          (d) => d.date === focusedData?.point?.date,
        );
        ...
        if (pointIndex !== -1) {
          ...
          if (
            tooltip.node()?.getAttribute('id') !==
            interactingChartId + '-tooltip'
          ) {
            tooltip.classed('proxy', true);
          }
...
export const ProgressBar = ({ className }: { className?: string }) => {
  ...
  const uxLoadingState = useAppState((state) =>
    Object.values(state.ux.chains).some((chain) => chain.isLoading),
  );
  const uxHandlers = storeHandlers().ux;
  ...
  return (
    ...
    >
      <div
        className={headerStyles.progressBarInner}
        style={{
          backgroundColor: tokens.kda.foundation.color.brand.primary.n50,
          width: `${progress}%`,
          transition: isResetting
            ? 'unset'
            : `width ${uxTimeState.refreshInterval}ms linear`,
        }}
      />
    </div>
  );
