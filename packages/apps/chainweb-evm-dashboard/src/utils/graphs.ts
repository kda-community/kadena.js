...
export function appendGraphTooltip(
  ...
  const tooltipExists =
    d3.select('body').selectAll(`#${chartId}-tooltip`).nodes().length > 0;
  let tooltip = null;

  if (tooltipExists) {
    tooltip = d3.select('body').select(`#${chartId}-tooltip`) as d3.Selection<
      HTMLDivElement,
      unknown,
      HTMLElement,
      any
...
export function moveGraphTooltip(
  ...
  if (d && typeof d.value === 'number' && tooltip) {
    ...
    const tooltipWidth =
      tooltipNode?.offsetWidth && tooltipNode.offsetWidth > 120
        ? tooltipNode.offsetWidth
        : 120;
    const pageWidth = window.innerWidth;
    const tooltipX =
      event.pageX + 10 + tooltipWidth > pageWidth
...
export function proxyGraphTooltip(
  ...
  if (d && typeof d.value === 'number' && tooltip) {
    ...
    const pageWidth = window.innerWidth;
    const tooltipX =
      focusX + 10 + tooltipWidth > pageWidth
